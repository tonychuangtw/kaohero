...
NEXT_ACTION: **114～108 年已全部完成（地特 422/774 卷、13,775/27,010 題、51.0%）**。下一步：**107 年，從三等第一卷 `loc-107-1-a001` 開始**，依三等（a*）→ 四等（b*）→ 五等（c*）順序做完，再接 106 → 105 → … → 102（尚餘 352 卷）。每卷流程：讀題 → Write 工具寫 patch JSON 到 scratchpad → chk.js → tools/set-exp.js --write → tools/build-index.js --write → test/test.js → commit + push。

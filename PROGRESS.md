...
NEXT_ACTION: **114／113／112／111／110 年已全部完成；109 年三等 18 卷也已完成**，四等做到 `b023`。下一步：**`loc-109-1-b024` 計算機概要（資訊處理組）** → `b025` → 再接 109 五等 `c003` 起，然後 108 → 107 → … → 102。每卷流程：讀題 → Write 工具寫 patch JSON 到 scratchpad → chk.js → tools/set-exp.js --write → tools/build-index.js --write → test/test.js → commit + push。

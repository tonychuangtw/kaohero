...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^(mlt|pt|ot|nut|tou)-，引擎 claude/claude-opus-5）。最後一卷：pt-110-1-pt2 110 年第一次　物理治療學概論，寫 76 題、跳過 4 題，09/19 15:11 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

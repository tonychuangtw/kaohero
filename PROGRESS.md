...
NEXT_ACTION: **主線＝高普考（`gao-*`，633 卷 17,995 題，目前 4,007 題／22.3%）**。115 年 1,327／1,345（98.7%）、114 年 1,184／1,220（97.0%）皆已完成；**現在做 113 年（1,018／1,220）**。113 年剩下的卷只有四大卷：`p023`（計算機概要電子工程組 40）、`p025`（動物解剖生理學概要 50）、`p027`（計算機概要資訊處理組 40）、`p030`（經濟學與財政學概要 50），另有零星未寫的題散在 `g002`／`g005`／`g012`／`g025`／`g029`／`p002`（多為圖表毀損或答案有爭議而刻意跳過者）。
...
UPDATED: 2026-09-11 台北（115、114 完成；113 做到 1,018／1,220）
...
- [ ] **擴充新科目（Tony 2026-09-09 定案的順序）**：現有八類的解析全部做完後，再一科一科加；每加一科就把該科解析寫完，才動下一科。順序：護理師 → 初等考試 → 警察特考 → 導遊領隊 → 其他醫事類（醫檢師、物理治療師、營養師、職能治療師）
  ...
  - 站上「題多解少」的缺口：地方特考 27,010 題只寫了 2,020；高普考 17,995 題已寫 4,600+（2026-09-11 這輪從 1,406 一路推上來）；教師檢定已完成

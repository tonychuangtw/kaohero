...
NEXT_ACTION: **114～103 年全部完成；102 年三等、四等全部完成，五等做到 c012**（全站 105,618/109,281 題、96.6%）。下一卷：**`loc-102-1-c013` 勞工行政與勞工法規大意**（五等 50 題），接著 c014～c029，102 年即收尾，再接 101 年。每卷流程：讀題（node -e 讀 js/data/exam/<pid>.js；圖片題用 Read 工具開 img/q/*.webp）→ Write 工具寫 patch JSON 到 scratchpad → node $SP/chk.js → node tools/set-exp.js <patch> --write → node tools/build-index.js --write → node test/test.js → git add 該卷與 js/data/exams.js → commit + push。工作區乾淨、已全部 push，沒有做到一半的卷。

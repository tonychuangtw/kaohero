...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^pol-，引擎 agy/gemini-3.8-flash-high）。最後一卷：pol-108-1-b003 108 年　三等考試　行政學，寫 25 題、跳過 0 題，09/16 16:27 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

...
NEXT_ACTION: **114～106 年全部完成，105 年三等（a001～a016）全部完成**（地特 551/774 卷、17,993/27,010 題、66.6%）。下一步：**105 年四等，從 `loc-105-1-b001` 開始**，做完接五等（c*），再接 104 → 103 → 102。每卷流程：讀題 → Write 工具寫 patch JSON 到 scratchpad → tools/set-exp.js --write → tools/build-index.js --write → test/test.js → commit + push。

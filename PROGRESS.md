...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 .，引擎 claude/claude-opus-5）。最後一卷：tcm-102-2-cmb2 102 年第二次　中醫基礎醫學（二），寫 6 題、跳過 0 題，09/24 05:37 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

...
NEXT_ACTION: **新科目「初等考試」已轉檔完成（2026-09-13）：385 卷、18,710 題、102～115 年**，全站現為 2,921 卷／139,271 題，已寫詳解 124,973 題。**下一步＝逐卷寫初等考試的詳解，由新到舊（115 年 27 卷 → 114 年 28 卷 → …→ 102 年 30 卷）。**
...
　**109 年已全部做完（2026-09-15）**（27 卷），只剩下列刻意跳過的題：`e030` 24 題、`e029` 15 題（電路圖／波形圖未隨轉檔保留）、`e017` #10 #30 #37（公式上標轉檔錯位、選項與波松計算不符）、`e019` #1 #27 #31、`e018` #47（二元樹圖未保留）、`e025` #15（排架號碼交錯毀損）、`e011` #16 #46 #50、`e031` #1（官方答案與現行條文衝突或兩選項皆可）。
　**下一步＝108 年**（`e031` 起由大到小共 27 卷、1,305 題），再往 107…一路到 102 年。

...
NEXT_ACTION: **114～104 年全部完成；103 年三等 a002～a017 全部完成，四等做到 b010**（全站 102,709/109,281 題、94.0%）。下一卷：**`loc-103-1-b011` 國籍與戶政法規概要**（四等 25 題），接著 b012～b025、五等 c002～c029，再接 102 年（68 卷）。每卷流程：讀題（node -e 讀 js/data/exam/<pid>.js；圖片題用 Read 工具開 img/q/*.webp）→ Write 工具寫 patch JSON 到 scratchpad → node $SP/chk.js → node tools/set-exp.js <patch> --write → node tools/build-index.js --write → node test/test.js → git add 該卷與 js/data/exams.js → commit + push。工作區乾淨、已全部 push，沒有做到一半的卷。

...
　　 已改成部分退回（壞的那題不寫、下批重挑），不會再因一題停批次。沒在跑就重跑同一行；撞額度會自己停
...
**② 英雄榜假資料**——移除無出處的「歷年上榜水準 78」基準線（不要再加回來）。
`BOARD_SEED` 先改 `false`，**同日 11:47 台北 Tony 要求「一樣先改成有，但加入規則讓它定時會跳動，
有真人分數上去時都再檢視一次然後盡量做真一點」**，於是改成下面這套（`js/app.js` 榜單段）：

| 規則 | 做法 | 參數 |
|---|---|---|
| 同一期內固定 | 每 24 小時一期，期內重整幾次都完全一樣 | `SEED_EPOCH_H = 24` |
| 跨期會跳動 | 每人分數 ±0~3 分漂移，6% 的人「重考進步」跳 4~10 分 | `seedScoreAt()` |
| 名單慢慢換血 | 110 人的種子池，每人有自己的出現期與停留 45~150 期 | `SEED_POOL/SEED_CYCLE/SEED_LIFE` |
| 依真人校準 | 每 1 個真人擠掉 2 個種子（約 25 人後歸零退場）；種子平均／離散度往真人靠攏；最高分被真人壓住 | `calibrateSeeds()` |

測試鉤子：`localStorage['kaohero.boardEpoch']` 可指定期數（重整後仍有效），`test/smoke.mjs` 靠它驗
「同一期固定／跨期跳動／跨一期仍留 47/50 人／隔 120 期會換血」。
⚠️ 排名要當付費賣點之前仍要確認種子已退場——這條沒有因為種子變真實而消失。
...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 .，引擎 claude/claude-opus-5）。最後一卷：chu-102-1-e021 102 年　初等考試　經濟學大意，寫 3 題、跳過 0 題，09/24 16:01 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->
...
## 最後一科完工紀錄（2026-09-20 03:28 台北）

worker 收工：281 卷 7,181 題、跳過 85、失敗 0 卷。這一段是 claude 引擎，範圍＝法規四科 106 卷
＋被清掉重寫的（條號 660 題、簡體 49 題）＋DeepSeek 讀不了圖放回的 296 題。

**其他醫事類這一科的總花費**：DeepSeek US$5.06（實付）＋ claude $224.69 等值（吃訂閱週限）。
claude 段分布：nut 88 卷 $73.64、pt 70 卷 $64.78、ot 64 卷 $48.14、mlt 38 卷 $17.42、tou 條號重寫 26 卷 $20.71。
（對照：導遊領隊整科 agy $0 ＋ DeepSeek $0.86 ＋ claude $168.42。）

**修正後的驗證**（兩項都要，以後重寫完照抄）：
```
# 殘留簡體字與中國法規：兩個都要是 0
node -e "...q.exp 比對 /[药养检医疗试验标规国际这见来说对时会长发现关点当门问题实组织细产业务员导师签级结构专区处进过还应无与]/ 與 /中國居民膳食|食品安全國家標準|衛健委|中華人民共和國/"
# 條號抽查：拿一個跨卷重複出現的考點，看不同卷是否一致且正確
```
結果：醫事類 43,521 題詳解，簡體 0、中國法規 0；條號抽查 `tou-104-1-l009 #10` 已寫「旅行業管理規則第 66 條」、
`tou-104-1-d002 #48` 寫「大陸地區人民來臺觀光許可辦法第 14 條（題幹本來就有）＋旅行業管理規則第 53 條」，正確。

## DeepSeek 停用與那批詳解的體檢（2026-09-19 Tony 定案＋全批掃描）

**Tony 09/19 11:40：「除非我有指定，不然不要再用 DeepSeek」**，11:59 補充「不用重寫，條號有缺陷修就好」。
已寫進 `CLAUDE.md`。預設引擎＝claude；agy／gemini flash 在 Claude 週限吃緊時仍可用。

**花費結算**：醫事類 530 卷 36,907 題＝**US$5.06**（餘額 18.51 → 13.45，每卷約 0.0095、每題約 0.00014 美金）。
加上 09/18 導遊領隊那批 0.86，儲值的 $20 用掉 6.55。今天跑在北京尖峰（週一～五 09–12、14–18 兩倍價），
所以每卷比 09/18 的 0.008 貴一些。

**「會不會查到大陸資料」全批掃描結果（36,805 題）**：方向是對的。
- 引用中國標準／法規（中國居民膳食指南、食品安全國家標準 GB、衛健委、國家衛生健康委、執業醫師法、中華人民共和國）：**0 題**
- 引用台灣機關與標準（衛福部、國健署、DRIs 國人膳食營養素參考攝取量、我的餐盤、疾管署、健保）：**420 題**
- 出現「中國」的 25 題全是內容本來就該提的：克山病（中國東北缺硒地方病）、白喉桿菌的「中國字排列」形態、
  G6PD Canton（廣東型）變異、2008 年中國毒奶粉三聚氰胺；另有一批其實是「開發**中國**家」被關鍵字誤抓。
  ⚠ 下次再掃要把 `開發中國家` 先排除，否則誤報一堆。

**真正抓到的缺陷：49 題混進簡體字**（34 卷），其中 `pt-102-1-pt4` 有 11 題整段都是簡體。
認法（這台沒有 opencc，用字元集判斷就夠）：
```
node -e "...q.exp.match(/[药养检医疗试验标规国际这见来说对时会长发现关点当门问题实组织细产业务员导师签级结构专区处进过还应无与]/)..."
```
⛔ 字元集不要放「繁簡同形」的字（胞、准、范、后、么、据…），我第一次把「胞」放進去，5,000 多題全誤報。
處理：`exp-clear.js` 用同一個字元集當 `--grep` 清掉那 49 題（備份 `tools/exp-clear-backup-simplified.json`），claude 重寫。

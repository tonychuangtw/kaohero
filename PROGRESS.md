...
NEXT_ACTION: **主線＝高普考（`gao-*`，633 卷 17,995 題）**。115～105 年皆已完成，**104 年高考（g004～g019）也已全部完成**（各卷剩下的都是圖片缺檔、圈碼／公式毀損、缺克漏字文章或官方答案與法條／教科書衝突而刻意跳過的題）。現在做 **104 年普考**：下一卷是 `gao-104-1-p003` 行政學概要 50 題，之後依序 p004→p027；104 做完接 103 → 102，最後做地方特考（`loc-*`）。每卷流程：node -e 讀題 →（有 fig 就用 Read 工具看 webp）→ 寫 patch JSON 到 scratchpad → 跑夾字／非中英數／JSON.parse 檢查 → `node tools/set-exp.js <patch> --write && node tools/build-index.js --write && node test/test.js` → commit+push。（Tony 2026-09-10 22:37 要求：每做完一批（約 8～10 卷或跨年度）用 telegram reply 回報一次進度。）
...
UPDATED: 2026-09-10 台北（104 年高考全部完成；下一卷 gao-104-1-p003）

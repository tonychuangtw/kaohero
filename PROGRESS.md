...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^(mlt|pt|ot|nut|tou)-，引擎 claude/claude-opus-5）。最後一卷：nut-111-1-nut6 111 年第一次　食品衛生與安全，寫 38 題、跳過 1 題，09/19 23:19 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

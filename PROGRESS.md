...
NEXT_ACTION: **114～105 年全部完成**（地特 596/774 卷、19,816/27,010 題、73.4%；全站 2,199/2,377 卷、100,096/109,281 題、91.6%，2026-09-12 全站詳解數突破 10 萬題）。現正做 **104 年**（63 卷、2,265 題，已完成 a001～a003 與 a002 補題）。下一卷：**`loc-104-1-a004` 行政法（一般行政組）**，接著 a005 經濟學、a006 民法、a007 稅務法規…依 a 系列（三等）→ b 系列（四等）→ c 系列（五等）順序做完 104 年，再接 103 → 102。每卷流程：讀題（node -e 讀 js/data/exam/<pid>.js；圖片題用 Read 工具開 img/q/*.webp）→ Write 工具寫 patch JSON 到 scratchpad → node tools/set-exp.js <patch> --write → node tools/build-index.js --write → node test/test.js → git add 該卷與 js/data/exams.js → commit + push。工作區乾淨、已全部 push，沒有做到一半的卷。

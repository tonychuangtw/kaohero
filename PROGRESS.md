...
NEXT_ACTION: **主線＝高普考（`gao-*`，633 卷 17,995 題）**。115～104 年皆已完成（104 年高考 g004～g019、普考 p003～p027 全部做完；各卷剩下的都是圖片缺檔、公式／結構式毀損、缺克漏字文章、官方廢題或兩個選項皆正確而刻意跳過的題）。現在做 **103 年**：下一卷是 `gao-103-1-g003` 行政學 25 題，之後依序 g004→g020、p003→p027（103-1-g001 國文 7/10、103-1-p002 法學知識與英文 43/50 已部分完成，其餘 0）；103 做完接 102，最後做地方特考（`loc-*`，27,010 題目前僅 2,020）。每卷流程：node -e 讀題 →（有 fig 就用 Read 工具看 webp）→ 寫 patch JSON 到 scratchpad → 跑夾字／非中英數／JSON.parse 檢查 → `node tools/set-exp.js <patch> --write && node tools/build-index.js --write && node test/test.js` → commit+push。（Tony 2026-09-10 22:37 要求：每做完一批（約 8～10 卷或跨年度）用 telegram reply 回報一次進度。）
...
高普考做完再做地方特考（loc，27,010 題只寫 2,020）
...
UPDATED: 2026-09-10 台北（104 年高普考全部完成，全站 80,010/109,281；下一卷 gao-103-1-g003）

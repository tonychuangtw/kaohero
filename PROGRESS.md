...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^(mlt|pt|ot|nut|tou)-，引擎 claude/claude-opus-5）。最後一卷：mlt-110-2-mlt2 110 年第二次　臨床血液學與血庫學，寫 1 題、跳過 0 題，09/20 02:57 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^(mlt-[0-9]+-[0-9]-mlt[1-6]|pt-[0-9]+-[0-9]-pt[13456]|ot-[0-9]+-[0-9]-ot[13456]|nut-[0-9]+-[0-9]-nut[1235])$，引擎 deepseek/deepseek-flash）。最後一卷：mlt-102-2-mlt5 102 年第二次　醫學分子檢驗學與臨床鏡檢學，寫 80 題、跳過 0 題，09/19 11:33 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

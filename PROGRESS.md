...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^chu-10[2-5]-）。最後一卷：chu-102-1-e017 102 年　初等考試　統計學大意，寫 40 題、跳過 0 題，09/14 21:20 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

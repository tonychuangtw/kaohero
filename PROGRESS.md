...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^pol-，引擎 agy/gemini-3.8-flash-high）。最後一卷：pol-107-1-c023 107 年　四等考試　犯罪學概要，寫 25 題、跳過 0 題，09/16 16:35 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

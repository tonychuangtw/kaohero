...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 .，引擎 claude/claude-opus-5）。最後一卷：tcm-108-1-cmc3 108 年第一次　中醫臨床醫學（三），寫 1 題、跳過 0 題，09/24 05:00 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

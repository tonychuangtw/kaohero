...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 .，引擎 claude/claude-opus-5）。最後一卷：mlt-109-2-mlt6 109 年第二次　微生物學與臨床微生物學，寫 2 題、跳過 0 題，09/24 08:49 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

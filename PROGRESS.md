...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^tou-，引擎 agy/gemini-3.8-flash-high）。最後一卷：tou-110-1-l004 110 年　領隊人員　外國語（英語），寫 66 題、跳過 9 題，09/17 19:47 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

...
NEXT_ACTION: **114～107 年全部完成**（地特 478/774 卷、15,658/27,010 題、58.0%）。下一步：**106 年，從三等 `loc-106-1-a001` 開始**，依三等（a*）→ 四等（b*）→ 五等（c*）做完，再接 105 → 104 → … → 102（尚餘 296 卷）。每卷流程：讀題 → Write 工具寫 patch JSON 到 scratchpad → tools/set-exp.js --write → tools/build-index.js --write → test/test.js → commit + push。107 年剩下的空題為無法作答者（英文克漏字題組文章轉檔遺失、官方答案與法條衝突），已記於各 commit。

...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^chu-10[2-5]-）。最後一卷：chu-102-1-e029 102 年　初等考試　電子學大意，寫 28 題、跳過 11 題，09/14 19:57 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

...
NEXT_ACTION: **114／113／112／111 年已全部完成**（111 年 2,217/2,285，其餘為官方答案有瑕疵或圖檔遺失而略過）。目前開始做 **110 年 61 卷**（a001～a018、b001～b026、c002～c029），做完再依序 109 → … → 102。每卷流程：讀題 → Write 工具寫 patch JSON 到 scratchpad → chk.js → tools/set-exp.js --write → tools/build-index.js --write → test/test.js → commit + push。

...
NEXT_ACTION: 教師檢定（`tea-*`，261 卷 8,098 題，已寫 5,022／62.0%；115～109、106、105、104、103、102 年已完成，**沒有 107、108 兩個年度的卷**，接著做 101 年）續做：打開 `js/data/exam/tea-101-1-t1001.js` 讀題（**閱讀測驗要用 CLAUDE.md 裡帶 `q.psg` 的那行指令**），寫 patch JSON → `node tools/set-exp.js <patch> --write && node tools/build-index.js --write && node test/test.js` → commit push；同年度 15 卷做完再往前一年。同年度的「教育理念與實務」四個類科考卷有大量重複題，先做一卷再跑 `python3 tools/reuse-exp.py <目標pid> <來源pid>...`（輸出目錄用環境變數 REUSE_OUT 指定，預設為當前目錄） 產生可直接套用的 patch。教檢做完再補高普考（gao，17,995 題只寫 1,406）、地方特考（loc，27,010 題只寫 2,020）
...
已完成 **115～109 年、106、105、104、103、102 年共 162 卷、5,022 題（62.0%）**。教檢**沒有 107、108 兩個年度的卷**（考選部那兩年未舉辦或題庫未收）。

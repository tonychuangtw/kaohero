...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^tou-，引擎 claude/claude-opus-5）。最後一卷：tou-107-1-l007 107 年　領隊人員　外國語（德語），寫 70 題、跳過 10 題，09/18 11:09 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

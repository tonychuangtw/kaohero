...
<!-- exp-worker:start -->
（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 ^(mlt|pt|ot|nut|tou)-，引擎 claude/claude-opus-5）。最後一卷：nut-103-2-nut4 103 年第二次　團體膳食設計與管理，寫 38 題、跳過 0 題，09/20 02:05 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。
<!-- exp-worker:end -->

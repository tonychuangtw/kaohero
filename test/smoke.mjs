...
/* 基準線不占名額（2026-09-11 Tony：「不含那兩個標準線要 50 人」；09-20 起只剩一條）。 */
ok(await ev(`document.querySelectorAll('#main .bd-row').length === 51`), '模考設定頁就看得到英雄榜');
...
ok((await ev(`[...document.querySelectorAll('#main .sec-h, #main h2, #main h3')]
   ...
   '榜單標題寫明是前 50 名');
/* 種子名次的三條規則（2026-09-20 Tony：「加入規則讓它定時會跳動」）：
   同一期內完全固定、跨期分數會動但人大致還是那批、真人越多種子越少。 */
const boardNow = async () => await ev(
  `[...document.querySelectorAll('#main .bd-row:not(.mark)')]
     .map(x=>x.querySelector('.bd-nk').textContent+':'+x.querySelector('.bd-sc').textContent).join('|')`);
const seedTop = await ev(`document.querySelector('#main .bd-row .bd-nk').textContent`);
const epoch0 = await boardNow();
await ev(`location.reload()`);
for (let i = 0; i < 120; i++) { await sleep(100); if (await ev('document.readyState === "complete"')) break; }
await sleep(500);
ok((await ev(`document.querySelector('#main .bd-row .bd-nk').textContent`)) === seedTop,
   '榜單內容穩定，重新載入不會換一批人');
ok((await boardNow()) === epoch0, '同一期內榜單完全固定');
const atEpoch = async (d) => {
  await ev(`localStorage.setItem('kaohero.boardEpoch', String(Math.floor(Date.now()/(24*3600*1000)) + ${d}))`);
  await ev(`location.reload()`);
  for (let i = 0; i < 120; i++) { await sleep(100); if (await ev('document.readyState === "complete"')) break; }
  await sleep(400);
  return await boardNow();
};
const epoch1 = await atEpoch(1);
ok(epoch1 !== epoch0, '換一期之後榜單會跳動');
const names = x => new Set(x.split('|').map(y => y.split(':')[0]));
const keep = [...names(epoch0)].filter(n => names(epoch1).has(n)).length;
ok(keep >= 40, '跨一期時大部分還是同一批人（留下 ' + keep + ' / 50）');
const far = await atEpoch(120);
ok([...names(epoch0)].filter(n => names(far).has(n)).length < 50, '隔很久之後名單會換血');
await ev(`localStorage.removeItem('kaohero.boardEpoch')`);
await ev(`location.reload()`);
for (let i = 0; i < 120; i++) { await sleep(100); if (await ev('document.readyState === "complete"')) break; }
await sleep(400);

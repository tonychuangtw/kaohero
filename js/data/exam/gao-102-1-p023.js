...
window.APP_EXAM_PAPERS['gao-102-1-p023'] = {
 ...
 "qs": [
  ...
  },
  {
   ...
   "a": 3,
   "exp": "✅ (D) 依分配律展開：A·(補數項 + ABC) = A·(補數項) + A·ABC。含補數的那一項與外層的 A 相乘時出現 A·Ā = 0，整項歸零；後項依等冪律 A·A = A，化簡後只剩 ABC，故答案為 ABC。\n❌ (A) BC + ABC 中的 BC 未與外層的 A 相乘，違反分配律；且 ABC 已包含 BC 的情形，此式也非最簡。\n❌ (B) BC 漏掉最外層必須保留的 A；只有在 A 被吸收律吸收時才可能成立，本式並非如此。\n❌ (C) 此選項為含補數的項，與外層的 A 相乘後為 0，不可能是化簡後保留下來的結果。\n📚 出處：數位邏輯—布林代數基本定律（分配律 A(B+C)=AB+AC、互補律 A·Ā=0、等冪律 A·A=A）。"
  },
  {
   "n": 12,
   "pt": 1,
  ...
  },
  {
   ...
   "a": 0,
   "exp": "✅ (A) 該二元樹以 A 為根，左子節點 B、右子節點 C，B 底下再接左子 D、右子 E。中序走訪的規則是「左子樹 → 根 → 右子樹」：先深入左子樹走到 D，回到 B，再走 B 的右子 E，接著回到根 A，最後走右子樹 C，順序為 D→B→E→A→C。\n❌ (B) ABCDE 是層序走訪（level-order，由上而下、由左而右逐層列出）的結果。\n❌ (C) ABDEC 是前序走訪（preorder，根 → 左 → 右）的結果。\n❌ (D) DEBCA 是後序走訪（postorder，左 → 右 → 根）的結果。\n📚 出處：資料結構—二元樹的走訪方式（inorder、preorder、postorder 與 level-order 的差異）。"
  },
  {
   "n": 18,
   "pt": 1,
  ...
  },
  {
   ...
   "exp": "✅ (D) 二元搜尋樹的定義：任一節點的左子樹所有鍵值都小於該節點，右子樹所有鍵值都大於該節點。圖中 a 是樹根，b 是左子節點、c 是右子節點，因此 b < a、a < c，合起來就是 b < a < c。\n❌ (A) a > b 雖然成立，但 a 是樹根、c 在右邊，必須 a < c，不可能 a > c，故 a > b > c 錯誤。\n❌ (B) a < b 與「左子樹鍵值小於樹根」相牴觸，左子節點 b 一定小於 a。\n❌ (C) b < c 確實成立（b < a < c），但 c < a 錯誤，右子節點必定大於樹根。\n📚 出處：資料結構－二元搜尋樹（Binary Search Tree）的定義與鍵值大小關係。"
  },
  {
   "n": 23,
   "pt": 1,
  ...
  },
  {
   ...
   "a": 2,
   "exp": "✅ (C) 圖中每個父節點的鍵值都大於其子節點（8 大於 7 與 5，7 大於 2），且為由上而下、由左而右填滿的完全二元樹，符合最大堆積（max heap）的兩項要件：完全二元樹＋父節點鍵值不小於子節點。\n❌ (A) 二元搜尋樹要求左子樹鍵值小於父節點、右子樹鍵值大於父節點；此處根 8 的右子為 5（小於 8），違反定義。\n❌ (B) 紅黑樹本身必須先是二元搜尋樹，另需節點著色並滿足根為黑、無連續紅節點、各路徑黑高相同等性質，圖中既非搜尋樹也無著色資訊。\n❌ (D) 最小堆積要求父節點鍵值不大於子節點，此處根 8 大於其子 7、5，恰好相反。\n📚 出處：資料結構—堆積（Heap）的定義與二元搜尋樹之比較。"
  },
  {
   "n": 26,
   "pt": 1,

...
window.APP_EXAM_PAPERS['loc-111-1-b023'] = {
 ...
 "qs": [
  ...
  },
  {
   ...
   "exp": "✅ (B) 計數器的 D3~D0 全接 0、Count 固定為 1，AND 閘取 Q3 與 Q0，當計數值為 1001（9）時 AND 輸出 1 並回授到 Load，下一個時脈就載入 0000，形成模 10 計數器，AND 輸出每 10 個 ICLK 才出現一個寬度 1 週期的脈波。該脈波再送入 XOR，XOR 另一端接 D 型正反器的輸出，構成「脈波到才翻轉」的 T 型正反器，所以 OCLK 每 10 個 ICLK 翻轉一次、每 20 個 ICLK 完成一個週期，頻率為 100 MHz ÷ 20 = 5 MHz。\n❌ (A) 1 MHz 相當於除 100，須模 50 的計數器再除 2，與本電路的模 10 不符。\n❌ (C) 10 MHz 是 AND 閘輸出（計數器的進位脈波）的頻率，那只是除 10，還要再經 T 型正反器除 2 才是 OCLK。\n❌ (D) 20 MHz 相當於除 5，等於忽略了 XOR 與 D 型正反器組成的除 2 級，且計數器模數是 10 不是 5。\n📚 出處：數位邏輯設計；同步計數器與除頻電路（模 N 計數器、T 型正反器除二）。"
  },
  {
   "n": 5,
   "pt": 1,
  ...
  },
  {
   ...
   "exp": "✅ (D) 以 Kruskal 由小而大挑邊並跳過會成環者：B-C 5、B-D 6、C-D 10（與前兩邊成環，捨去）、B-F 12、F-D 14（成環，捨去）、A-B 16、E-F 18，此時已用 5 條邊連通全部 6 個頂點（n−1 = 5），總成本 5 + 6 + 12 + 16 + 18 = 57。\n❌ (A) 47 是把最小的 5 條邊 5、6、10、12、14 直接相加，但其中含 B-C-D-B 與 B-F-D-B 兩個環，且 A、E 根本沒被連上，不是生成樹。\n❌ (B) 58 對應改用 A-E 19 而非 E-F 18 去連接 E，雖然也是合法的生成樹，但多了 1 的成本，並非最小。\n❌ (C) 52 低於 57，而 57 已是連通 6 個頂點所需的最小總成本，任何合法生成樹都不可能更低。\n📚 出處：資料結構與演算法；最小成本生成樹（Kruskal 演算法、避圈法）。"
  },
  {
   "n": 18,
   "pt": 1,

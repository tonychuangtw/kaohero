...
window.APP_EXAM_PAPERS['loc-104-1-b023'] = {
 ...
 "qs": [
  ...
  },
  {
   ...
   "exp": "✅ (C) 圖中 Count 接 1（永遠計數）、資料輸入 D3D2D1D0 全接 0，而 Q2 與 Q0 經 AND 閘接回 Load。計數器從 0000 開始往上數，數到 Q2=1 且 Q0=1（即 Q=0101=5）時 AND 輸出為 1、Load 被拉起，下一個時脈邊緣就把 0000 載入而非加 1。因此實際出現的狀態為 0、1、2、3、4、5 共 6 個，循環長度 N=6。\n❌ (A) 4 要在數到 3（0011）時偵測並重載，偵測邏輯應取 Q1·Q0，與圖中取 Q2·Q0 不符。\n❌ (B) 5 表示狀態只有 0～4；但 Q=0100（4）時 Q0=0，AND 輸出為 0，不會重載，計數會繼續到 5。\n❌ (D) 7 表示數到 6（0110）才重載；6 的 Q0=0 無法讓 AND 輸出為 1，且計數在 5 時就已被拉回 0，根本到不了 6。\n📚 出處：數位邏輯設計—同步計數器與 modulo-N 計數器（以並行載入 Load 偵測終止狀態實現除頻）"
  },
  {
   "n": 8,
   "pt": 1,

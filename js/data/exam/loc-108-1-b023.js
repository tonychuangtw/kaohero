...
window.APP_EXAM_PAPERS['loc-108-1-b023'] = {
 ...
 "qs": [
  ...
  },
  {
   ...
   "exp": "✅ (B) 圖由兩塊完全圖以一條橋 d–e 串起：左邊 a、b、c、d 四點兩兩相連為 K₄，右邊 e、f、g、h、i 五點兩兩相連為 K₅。由 Cayley 公式，Kₙ 的生成樹數為 n^(n−2)，故 K₄ 有 4²＝16 種、K₅ 有 5³＝125 種；橋 d–e 是唯一連通兩塊的邊，必定被選入而只有 1 種選法。總數＝16×125×1＝2000 種。\n❌ (A) 1440 不是 16×125 的結果，與兩塊完全圖相乘的數目不符。\n❌ (C) 2880 相當於把某一邊的生成樹數目算錯（如誤用 4³＝64 之類），高於實際值。\n❌ (D) 4200 遠大於正確數目，亦非 16 與 125 的乘積。\n📚 出處：圖論—生成樹計數與 Cayley 公式（完全圖生成樹數 n^(n−2)、橋與連通分量的乘法原理）。"
  },
  {
   "n": 22,
   "pt": 1,
  ...
  },
  {
   ...
   "exp": "✅ (C) 圖中 6 個頂點、3 條邊，且每條邊互不相接（為三組獨立的邊）。可把每條邊的兩端分別放進兩個集合 X、Y，使所有邊都橫跨 X 與 Y、集合內部沒有邊，符合二分圖的定義。（事實上任何沒有奇數長度迴路的圖都是二分圖，此圖無迴路故必為二分圖。）\n❌ (A) 完全圖要求任兩頂點間都有邊，6 點的完全圖需 6×5/2＝15 條邊，此圖只有 3 條。\n❌ (B) 樹必須連通且邊數＝頂點數−1（此處應為 5 條）；本圖不連通、只有 3 條邊，屬於森林而非樹。\n❌ (D) 連結圖要求任兩頂點間都存在路徑，本圖分成三個彼此不相通的部分，並不連通。\n📚 出處：圖論—圖的分類（完全圖、樹、二分圖、連通圖的定義與判別）。"
  },
  {
   "n": 38,
   "pt": 1,

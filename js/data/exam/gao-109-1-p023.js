...
window.APP_EXAM_PAPERS['gao-109-1-p023'] = {
 ...
 "qs": [
  ...
  },
  {
   ...
   "a": 0,
   "fig": "img/q/109090_450_1416_22.webp",
   "exp": "✅ (A) 9 個頂點的生成樹需 9−1＝8 條邊。依 Kruskal 由小到大取：權重 3 的邊有 a-d、b-c、c-d、d-e、e-i、f-g、f-h 共 7 條，依序加入都不成環，此時連成 {a,b,c,d,e,i} 與 {f,g,h} 兩群；再用一條權重 4 的 c-h（或 i-h）把兩群接起來即完成，總長＝3×7＋4＝25。\n❌ (B) 26 比最佳解多 1，是把某條權重 3 的邊換成權重 4 的邊（例如用 g-h 取代 f-h）的結果，仍可再縮短。\n❌ (C) 27 多算了一條權重 4 的邊（如 c-h 與 i-h 同時納入），兩群之間出現兩條橋，已形成環路而非樹。\n❌ (D) 28 是誤納 a-b 或 f-i 這類權重 5 的邊所致；這兩條邊的兩端都已被權重較小的邊連通，不該選入。\n📚 出處：資料結構－圖形的最小生成樹（Kruskal 演算法）。"
  },
  {
   "n": 23,
   "pt": 1,

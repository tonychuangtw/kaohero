...
window.APP_EXAM_PAPERS['gao-108-1-p026'] = {
 ...
 "qs": [
  ...
  },
  {
   ...
   "exp": "✅ (C) 拓樸排序要求每個節點都排在它所有前行節點（指向它的節點）之後。c、a 是入度 0 的起點，先排 c、a，再排 b、d，接著才是 e；e 指向 g、f、i、h，因此 e 必須在它們之前，而 h→k 也讓 h 排在 k 前面，整串順序都不違反任何一條有向邊，是合法的 topological order。\n❌ (A) 把 f 排在 e 前面，但圖中有 e→f，e 尚未輸出就先輸出 f，違反拓樸排序。\n❌ (B) 把 g 排在 e 前面，但 e→g；末段又把 k 排在 i、h 之前，而 h→k，兩處都違反。\n❌ (D) 把 k 排在 h、i 之前，但 h→k、i→h；且把起點 c 與 g 丟到最尾端，與 e→g 的先後關係不合。\n📚 出處：資料結構－有向無環圖（DAG）的拓樸排序（topological sort）。"
  },
  {
   "n": 16,
   "pt": 1,
   ...
   "exp": "✅ (A) 後序走訪的順序是「左子樹→右子樹→樹根」。此運算式樹樹根為 +，左子樹是 1，右子樹是以 * 為根、左邊接 (6−8)、右邊接 8。依序展開：左子樹得 1；右子樹先走 (6−8) 得「6 8 −」，再走 8，最後輸出 *，得「6 8 − 8 *」；最後輸出根 +。合起來即「1 6 8 − 8 *  +」＝ 168-8*+。\n❌ (B) 「1-6*8+1」把運算子夾在運算元中間，是中序的寫法，而且運算元 1 出現兩次，與樹的內容不符。\n❌ (C) 「68-8*1+」把 1 放到最後，代表 1 是根 + 的右子樹；但圖中 1 在根的左邊，後序時左子樹最先走，1 必須排在最前面。\n❌ (D) 「1868-*+」的運算元順序為 1、8、6、8，與樹由左至右的 1、6、8、8 不同，且 8 排在 6 之前不符合 (6−8) 的子樹結構。\n📚 出處：資料結構－二元樹的後序走訪（postorder traversal）與運算式樹（expression tree）。"
  },
  {
   "n": 17,
   "pt": 1,
  ...
  },
  {
   ...
   "exp": "✅ (D) 圖由 d–e 這條橋接起兩塊，橋一定在每棵生成樹裡，所以總數＝左塊生成樹數 × 右塊生成樹數。左塊 a、b、c、d 有 ab、ac、ad、bc、cd 共 5 邊，等於 K4 少一條 bd；K4 有 16 棵生成樹，每條邊出現在 16×3/6＝8 棵中，故扣掉含 bd 的 8 棵剩 8 棵。右塊是 f 與 i 之間有三條獨立路徑（f–i、f–e–i、f–g–h–i，長度 1、2、3）的 theta 圖，生成樹數＝1×2＋2×3＋3×1＝11。總數 8×11＝88。\n❌ (A) 60 相當於把左塊誤算成 6 棵、右塊誤算成 10 棵；左塊實際 8 棵、右塊 11 棵。\n❌ (B) 66 是把左塊誤算成 6 棵（6×11）的結果，K4 去掉一邊應為 16−8＝8 棵。\n❌ (C) 80 是把右塊誤算成 10 棵（8×10）的結果，三條路徑長 1、2、3 的 theta 圖應為 1×2＋2×3＋3×1＝11 棵。\n📚 出處：離散數學／圖論－生成樹計數（橋的分解與 theta 圖的生成樹數 ab＋bc＋ca）。"
  },
  {
   "n": 22,
   "pt": 1,

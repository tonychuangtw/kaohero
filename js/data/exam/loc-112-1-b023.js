...
window.APP_EXAM_PAPERS['loc-112-1-b023'] = {
 ...
 "qs": [
  ...
  },
  {
   ...
   "a": 2,
   "fig": "img/q/112200_439_2712_14.webp",
   "exp": "✅ (C) 陣列從索引 1 開始存，現有 11 個元素（索引 1~10），故 70 先放進索引 11，再往上浮（percolate up）。索引 11 的父節點是 11/2 = 5（值 38），70 > 38 交換，70 移到索引 5；索引 5 的父節點是 2（值 60），70 > 60 再交換，70 移到索引 2；索引 2 的父節點是 1（值 88），88 > 70 不再交換。最後 70 停在索引 2。\n❌ (A) 11 只是插入時暫時擺放的位置，因 70 大於父節點 38，必須繼續上浮，不會留在此處。\n❌ (B) 5 是只交換一次後的位置，但 70 仍大於父節點 60，還要再上浮一層。\n❌ (D) 索引 1 是根節點，最大堆積的根存最大值 88；88 > 70，70 不可能升到根。\n📚 出處：資料結構；最大堆積（max-heap）以陣列表示法的插入與上浮（percolate up）調整。"
  },
  {
   "n": 15,
   "pt": 1,
  ...
  },
  {
   ...
   "a": 0,
   "fig": "img/q/112200_439_2712_17.webp",
   "exp": "✅ (A) 圖中相鄰關係為 1:{2,6,7}、2:{1,3,4,7}、3:{2,4,8}、4:{2,3,5,8}、5:{4,6,8}、6:{1,5,7}、7:{1,2,6,8}、8:{3,4,5,7}。DFS 每次挑編號最小的未走訪鄰點：1→2→3→4→5→6→7→8，八個節點恰好一路深入不需回溯，順序為 1 2 3 4 5 6 7 8。\n❌ (B) 走到 3 時未走訪鄰點有 4 與 8，依規則要先選較小的 4，不能跳到 8。\n❌ (C) 節點 2 的鄰點是 1、3、4、7，並不含 6，DFS 無法由 2 直接走到 6。\n❌ (D) 同樣違反 2 與 6 不相鄰的事實，且 1 開始時最小未走訪鄰點是 2 之後應續深入 3，不會出現 6 7 3 的走法。\n📚 出處：資料結構；圖形的深度優先搜尋（DFS）走訪順序與鄰接關係判讀。"
  },
  {
   "n": 18,
   "pt": 1,

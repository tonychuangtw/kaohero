...
while :; do
  ...
  # 剩下的候選模型判斷不該寫（多半是轉檔毀損、不是法規衝突）→ 同一批會一直被挑到、一直寫 0。
  # 2026-09-24 就這樣空轉 180 批（約 9 小時 Claude 額度）才被發現，所以寫 0 就收工。
  [ "$got" -eq 0 ] && { echo "$(now) 本批寫 0 題：剩下 ${left:-?} 題模型都判定不寫，收工" | tee -a "$LOG"; break; }

...
while :; do
  ...
  last_year="$year"
  rm -f "$T"/patch.json "$T"/skip.json "$T"/out.json
  node tools/exp-dump.js "$pid" > "$T/q.txt" 2>/dev/null
  sed -e "s|__ROOT__|$ROOT|g" -e "s|__PID__|$pid|g" -e "s|__T__|$T|g" tools/exp-prompt.md | sed -e "/__QUESTIONS__/{r $T/q.txt" -e 'd}' > "$T/prompt.md"
  t0=$(date +%s)
...
print(f"in={i//1000}k out={u.get('output_tokens',0)//1000}k cost=${d.get('total_cost_usd',0):.2f} turns={d.get('num_turns','?')}")
PY
)
  # 撞額度／登入問題：不算失敗，等 30 分再試同一卷
  if grep -qiE 'usage limit|rate limit|overloaded|login|credit balance|authentication' "$T/err.txt" "$T/out.json" 2>/dev/null && [ ! -s "$T/patch.json" ]; then
  ...
  python3 - "$ROOT/PROGRESS.md" "$pid" "$title" "$n_written" "$n_skip" "$(now)" "$MATCH" <<'PY'
import sys,re
f,pid,title,w,s,ts,match=sys.argv[1:]
txt=open(f,encoding='utf-8').read()
block=f"<!-- exp-worker:start -->\n（自動更新，勿手改）詳解批次由 tools/exp-worker.sh 逐卷開新 session 執行（範圍 {match}）。最後一卷：{pid} {title}，寫 {w} 題、跳過 {s} 題，{ts} 台北。跳過的題記在 tools/exp-skips.json；失敗的卷在 ~/.claude/exp-worker.failed；每卷紀錄 ~/.claude/exp-worker.log。\n<!-- exp-worker:end -->"
...
worker: $usage ${secs}s model=$MODEL" 2>/dev/null
